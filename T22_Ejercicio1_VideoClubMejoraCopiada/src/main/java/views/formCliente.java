	...
	public formCliente() {
		...
		btnGuardar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirmacion = JOptionPane.showConfirmDialog(null, "¿Está seguro de guardar este cliente?",
						"Confirmar Guardar Cliente", JOptionPane.YES_NO_OPTION);
				if (confirmacion == JOptionPane.YES_OPTION) {
					cliente objetoCliente = new cliente();
					objetoCliente.insertarCliente(textField_3Nombre, textField_1Apellido, textField_4Dirección,
							textFieldDNI, textField_2Fecha);
		...
		// Ajustar la asignación de campos en el método actionPerformed del botón
		// "Eliminar"
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent event) {
				if (!event.getValueIsAdjusting()) {
					cliente objetoCliente = new cliente();
					objetoCliente.SeleccionarCliente(table, textFieldId, textField_3Nombre, textField_1Apellido,
							textField_4Dirección, textFieldDNI, textField_2Fecha);
				}
			}
		});
		btnModificar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirmacion = JOptionPane.showConfirmDialog(null, "¿Está seguro de modificar este cliente?",
						"Confirmar Modificar Cliente", JOptionPane.YES_NO_OPTION);
				if (confirmacion == JOptionPane.YES_OPTION) {
					cliente objetoCliente = new cliente();
					objetoCliente.ModificarCliente(table, textField_3Nombre, textField_1Apellido, textField_4Dirección,
							textFieldDNI, textField_2Fecha, textFieldId);
					objetoCliente.MostrarCliente(table);
				}
			}
		});
